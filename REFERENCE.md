...
These are protocol-agnostic; they run the same regardless of which protocol module a resource belongs to. Protocol-specific steps (`http_fetch`, `http_sse`, `html`/`markdown`/`json`, `http_response`, `http_headers`/`http_cookies`, `http_redirect`/`http_reroute`) are documented with the protocol that provides them; see [Protocols → http](#http).
...
Built-in regex macros are defined in `nerak.h`; define your own the same way: `#define zipcode_input "^\\d{5}$"`.
...
**Built-in validators:**
- Strings: `not_empty_input`, `alpha_input`, `alphanumeric_input`, `slug_input`, `no_html_input`
...
- Web: `url_input`, `ipv4_input`, `hex_color_input`
...
For `GET /todos/5` the executed order is: `input({"id", ...})` (resource `.all`), `logged_in()`, `session()` (module `middleware`), then the verb pipeline `sqlite_query({"get_todo", ..., .error_on_empty = true})`, `html("todo", "todo_s")`, `http_response("todo_s")`.
...
Nerak is resource-based, not route-based. Each `http(...)` defines a named resource and URL with HTTP verb pipelines. `{{url:name}}`, `http_redirect()`, and `http_reroute()` all take only the resource name; `:params` are read from the current scope by matching key names. Path specificity is automatic: exact matches (`/todos/active`) take priority over parameterized matches (`/todos/:id`) regardless of definition order.

Clients select a verb via the request method, or by passing `http_method` as a query/form parameter. This lets HTML forms (limited to GET/POST) reach any verb, and gives SSE a connection path: `/todos?http_method=events`. Templates emit it via `{{http_verb:input}}` / `{{http_verb:param}}` (see [Templates](#templates)).

**Resource name *(by order)***: identifier used by `{{url:name}}`, `http_redirect()`, and `http_reroute()`.
```c
...
**`.sse`**: persistent SSE channel. The first value is the channel name (supports `{{interpolation}}`); any remaining steps run on connect. Clients reach this pipeline with the `events` verb (`?http_method=events`, or `{{http_events:param}}` / `{{http_events:input}}`).
```c
http("todos", "/todos",
  .sse = {"todos:{{user_id}}",
    sqlite_query({"db", "get_todos", "todos_data"}),
...
**`{{http_verb:param}}`**: emit an `http_method` override for URL query strings, letting a link reach a non-GET verb. One per verb: `{{http_get:param}}`, `{{http_post:param}}`, `{{http_put:param}}`, `{{http_patch:param}}`, `{{http_delete:param}}`, `{{http_events:param}}`; each outputs `http_method=<verb>`. See [HTTP Pipelines](#http-pipelines).
```html
<a href='{{url:todo}}?{{http_delete:param}}'>Delete</a> <!-- /todos/:id?http_method=delete -->
```

**`{{http_verb:input}}`**: emit a hidden `<input>` carrying the `http_method` override, letting a `<form>` (GET/POST only) reach any verb. One per verb: `{{http_get:input}}`, `{{http_post:input}}`, `{{http_put:input}}`, `{{http_patch:input}}`, `{{http_delete:input}}`, `{{http_events:input}}`; each outputs `<input type='hidden' name='http_method' value='<verb>'>`.
```html
...
```c
http_fetch({.url = "https://api.dev/charge", .context_key = "receipt", .method = http_post, .json_ctx_key = "order"})
```
...
```c
http("todos", "/todos", .all = {logged_in()}, .get = { ... });
```
...
http("logout", "/logout", .post = {logout()});
http("signup", "/signup",
  .get = {
